...
const CompletedEvent = () => {
  ...
  // Filter and map events that have already occurred
  const completedEvents = userData.organiserEvents.filter((event) => {
    const eventDate =
    ...
    <View style={styles.container}>
      {completedEvents.length === 0 ? (
        <View style={styles.animationContainer}>
          ...
          />
          <Text style={styles.noTicketsText}>No completed events found!</Text>
        </View>
      ) : (
        <FlatList
