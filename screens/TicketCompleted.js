...
const TicketCompleted = () => {
  ...
  // Filter and map events with "Completed" booking status
  const completedEvents = userData.userEvents.filter((event) =>
    userData.bookedTickets.some(
      (ticket) =>
        ticket.eventId === event.id && ticket.bookingStatus === "Completed"
    )
  );
